...
class DepthMapper(Node):
    def __init__(self):
        ...
        # ---- Pubs & Subs
        # We must publish CameraInfo alongside Depth for pointclouds to work
    ...
    def info_cb(self, msg):
        if self.cam_info_msg is None:
            ...
            self.focal = msg.k[0]
            # We can stop listening to info once we have it, but we MUST keep republishing it
            self.destroy_subscription(self.info_sub)
            self.get_logger().info(f"CamInfo Locked. Focal: {self.focal:.2f} px")
    ...
    def update_baseline(self):
        ...
        try:
            ...
            if self.focal:
                ...
                self.get_logger().info(f"Baseline Locked: {self.baseline:.4f} m")
                return True
    ...
    def stereo_cb(self, left_msg, right_msg):
        ...
        depth_msg.header = left_msg.header # CRITICAL: timestamp must match left image
        self.depth_pub.publish(depth_msg)

        # 4. Publish Camera Info (CRITICAL for pointcloud sync)
        # We reuse the cached info msg, but update the header to match the current frame
